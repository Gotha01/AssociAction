...
def home(request):
    ...
    if request.method == 'POST':
        ...
        if (form.is_valid()
            or form.cleaned_data.get('quoi')
                or form.cleaned_data.get('ou')):
            ...
            if quoi:
                associations = associations.filter(
                    Q(associationname__icontains=quoi) | Q(
                        associationsector__sector__sectorname__icontains=quoi)
                )
...
def contact(request):
    ...
    if request.user.is_authenticated:
        user_role = UserRoleAssociation.objects.filter(
            role=1,
            user=request.user
        )
        if user_role:
