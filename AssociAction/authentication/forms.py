...
class RegistrationForm(forms.Form):
    name_validator = RegexValidator(
        regex=r'^[a-zA-Z\-]+$',
        message="Pour les noms et prénoms, entrez des lettres, seul '-' est toléré."
    )
    last_name = forms.CharField(max_length=50, label="Nom", validators=[name_validator])
    first_name = forms.CharField(max_length=50, label="Prénom", validators=[name_validator])
    username = forms.CharField(max_length=50, label="Nom d'utilisateur")
    email = forms.EmailField(max_length=100, label="Adresse e-mail")
    password = forms.CharField(widget=forms.PasswordInput, label="Mot de passe")
    password_confirmation = forms.CharField(widget=forms.PasswordInput, label="Confirmer le mot de passe")
