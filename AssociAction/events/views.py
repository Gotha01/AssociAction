    ...
    def decorator(view_func):
        ...
        def _wrapped_view(request, association_id, *args, **kwargs):
            try:
                ...
                user_role = UserRoleAssociation.objects.get(user=request.user, association=association).role
...
def create_event(request, association_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('event_address', association_id=association_id, event_id=event.id)
    else:
        form = EventForm()
...
def event_detail(request, association_id, event_id):
    ...
    try: 
        have_roles = UserRoleAssociation.objects.get(user=request.user,association=association)
        if have_roles.role in roles:
            user_resp = True
            return render(request, 'events/event_detail.html', {'event': event, 'association':association, 'address':address, "user_resp":user_resp})
    ...
    return render(request, 'events/event_detail.html', {'event': event, 'association':association, 'address':address})
...
def event_update(request, association_id, event_id):
    ...
    if request.method == 'POST':
        ...
        if event_form.is_valid():
            ...
            return redirect('event_detail', association_id=association_id, event_id=event_id)
...
def event_address(request, association_id, event_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            EventAddress.objects.create(event=actual_event, address=new_address)
            return redirect('event_detail', association_id=association_id, event_id=event_id)
